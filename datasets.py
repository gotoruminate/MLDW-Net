...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        ...
        img = hdf5storage.loadmat(folder + 'source.mat')['ori_data']

        rgb_bands = [1,2,3]

        gt = hdf5storage.loadmat(folder + 'source_gt.mat')['map']

        label_values = ["cropland", "forest", "grassland",
                        "water", "building"]
        ignored_labels = [0]

    elif dataset_name == 'goal':
        img = hdf5storage.loadmat(folder + 'goal1.mat')['ori_data']

        rgb_bands = [1, 2, 3]

        gt = hdf5storage.loadmat(folder + 'goal1_gt.mat')['map']
